...
*Multi-complaint gap-return* — Om's own proposed resolution, likely but not yet confirmed with the owner: ₹350 once (shared, not per complaint) + the normal per-complaint therapy rate for each complaint being treated, same as an ordinary multi-complaint day. Recommended build: model the 350 as an additive line item (added once whenever "first billing touchpoint after a gap" is true) rather than a separate third pricing branch. The same two rules that already handle a normal multi-complaint day — add 350 once if it's a first-touchpoint event, add the per-complaint rate for each complaint treated — handle every gap-return combination automatically, with no new formula needed per complaint-count.

*Waiver mechanism* — recommended approach: a request/approve flow, not a shared-session or password-prompt flow. Receptionist submits at full price with a `waiver_requested` flag; a separate role-gated action (only `role = 'admin'`, same pattern as `validate_owner_is_admin()` elsewhere in this schema) is the only thing allowed to change the charged amount or clear the flag. Rejected: recreating the visit from scratch (throws away the receptionist's entry, needs the admin physically present) and a Windows-UAC-style password prompt typed into the receptionist's active session (would either silently swap which user's session is active, or require building a standalone step-up-auth mechanism Supabase doesn't provide out of the box).
