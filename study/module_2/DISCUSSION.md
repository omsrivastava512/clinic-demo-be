...
<!-- Custom instruction to enforce naming convention omitted for maintaining relevance -->
...
## The M2-04 question — no contradiction, two different topics

Worth resolving completely rather than leaving any doubt, and I checked back through the whole thing to be sure before answering.

M2-04 genuinely did cover dependency ordering — that's real, it's the capstone section at the end of that response, and it's exactly what "Shape 1" was citing. It walked through the full table-creation sequence and closed with: *"you can't insert a patient before the admin profile that owns it exists, can't insert a visit before its complaint course exists, can't insert an invoice before the patient has a `patient_clinic_access` row at that clinic."* That's a direct, accurate source for Shape 1's ordering (complaint_courses → visits → visit_services → invoices).

M2-04 did not cover orphan-stripping, `ProcedureLogger`, or `validateInvoiceItems` anywhere. Its actual eight sections were: the one-to-one relationship via shared primary key (`profiles`/`auth.users`), the circular FK between `clinics` and `profiles`, multiple FKs to the same table meaning multiple relationships, NULL semantics in FKs and uniqueness, composite UNIQUE constraints as FK-scoped uniqueness, the rule that a FK always lives on the many side, the denormalization-drift gap, and the dependency-ordering capstone. Nothing about frontend billing safeguards anywhere in there.

So: no contradiction. I cited M2-04 correctly for dependency ordering, and separately, correctly said it doesn't cover orphan-stripping — two different, specific claims about two different pieces of content, both true at once.

## Where orphan-stripping actually happens

Direct answer: frontend, and yes, it looks like it's already implemented — as real, existing client-side code, entirely independent of whether any backend integration exists yet.

Per what `codebase-context.md` describes: a function named `validateInvoiceItems`, living inside `ProcedureLogger/index.tsx`. A `.tsx` file is unambiguously a React component file — there's no version of this that's backend or database code. It operates on local, in-browser state (the `complaintId::procedureId` map of whatever's currently logged), stripping out any entry whose parent complaint has since been unchecked, right before that state becomes whatever gets submitted next.

Why it makes sense that this is already done while the backend isn't hooked up at all: this safeguard's whole job — cleaning up local editing state before it turns into a payload — doesn't need a backend to exist or function. It's fully self-contained, operating purely on data already sitting in the browser. It can run exactly the same way against your current mock data as it eventually will against real data, completely decoupled from whether "submit" currently does nothing, fakes a response, or someday actually calls Supabase.

One honesty flag: this is read from `codebase-context.md`'s description, which names the function and file specifically enough that I'm confident it's documenting real, existing code rather than a planned feature — but I haven't seen the literal source. Opening that exact file is the way to confirm for certain.

## Frontend role access — full brainstorm

One line I want to draw clearly before going deep: literal implementation — actual React routing code, Supabase Auth session handling, how a component tree consumes any of this — is exactly what Module 10 defers, and I'm holding that line even under "as much detail as possible," because that's a boundary you set up deliberately, not something I'm inventing now. But underneath that, there's a genuinely rich architecture question — what should differ per role, and what data actually drives the decision — and that side gets the full treatment.

**The one fact everything else hangs off.** After login, the app needs exactly one thing before it can decide anything: the logged-in user's own `profiles` row. This is a direct payoff of the one-to-one relationship from M2-04 — because `profiles.id` *is* `auth.users.id`, not a separately generated id needing its own lookup, there's no searching involved. The moment you know who's logged in, you already know which profile is theirs. One read, `role` and `clinic_id` come back, and every subsequent UI decision — what to show, what to scope queries to, whether a clinic-picker even makes sense — is a plain conditional on those two values. No new tables, no new backend logic beyond ordinary scoped reads you already know how to write.

**Receptionist** (`role = 'receptionist'`, `clinic_id` always set): clean and confident. Exactly one legitimate clinic to be looking at — their own — so auto-scoping directly to `profiles.clinic_id` on login, zero picker shown, is correct. There's nothing to choose between; a picker with one greyed-out option would just be noise.

**Admin** (`role = 'admin'`, `clinic_id` always `NULL`): this is the M2-05 clinic-switcher design applying directly, nothing new to derive — chain-wide visibility by default, since RLS already grants that unconditionally, with the switcher as a pure display-narrowing convenience on top, never a security boundary.

**Clinician** (`role = 'clinician'`, `clinic_id` set) — and here I want to actually engage with your uncertainty rather than smooth it over, because I think it's well-founded. Going back through everything `codebase-context.md` describes — patient intake, clinical notes, procedure logging, invoicing, the daily ledger — none of it reads as a distinctly clinician-operated screen. It reads as front-desk-operated data entry, including the clinical content itself. Given that, my honest read: `clinician_id` and the `clinician` role plausibly function today as an *attribution* field — who's the treating clinician, recorded by whoever's actually running the screen — rather than as evidence clinicians are meant to log in and operate the app themselves.

If that's accurate, my working recommendation: for now, a clinician sees essentially the same interface as a receptionist — same clinic-scoped tools, same auto-scoping via their own `profiles.clinic_id` — with one practical upside: when a clinician *is* the one submitting a form, `clinician_id`/`created_by` gets set to their own id automatically, no dropdown needed. That's not a new idea out of nowhere — it directly matches something already sitting in your own notes: "no clinician selector on procedure logger/registration — deliberate for single-clinician clinic." A logged-in clinician submitting their own work is exactly the case where that selector becomes unnecessary by construction.

Flagging clearly: this is an inference from a documentation summary, not a confirmed fact. You'd know the real, current feature list far better than I can reconstruct from a description — if there's a genuinely clinician-only screen or action that doesn't come through in that doc, this read is wrong and worth correcting directly.

**The future multi-clinic-clinician case**, kept brief since you've parked it: when this becomes real, the shape is one you already fully understand — `profiles.clinic_id` being a single column can't represent "this clinician works at three branches" any more than a single `patients.clinic_id` could represent multi-branch patients, which is exactly why that got removed back in v4. The fix would be structurally identical to `patient_clinic_access` — a `clinician_clinic_access`-style junction, same many-to-many shape, different relationship. Not designing it now, just noting it's not a new category of problem when it does come up.

## Backend validation — approaches, compared, with a real recommendation

**Approach 1 — lean on the atomic submission function alone, add nothing else.** Already where the earlier reasoning was heading: one function, one transaction, everything for a session created together. *Pro:* zero new schema, nothing new to design or maintain. *Con:* doesn't validate what's actually in the submission — narrows the window for tampering (nothing can be inserted separately, later, and quietly bundled in) but a single malicious or buggy call could still include an illegitimate complaint in its own parameter list. Cheapest, and genuinely the weakest.

**Approach 2 — a real, persisted session/encounter table.** The actual structural fix: a new table, one row per sitting, created atomically at submission time, holding exactly which `complaint_course_id`s were legitimately included. Every `visits` row from that submission carries a `session_id` back to it, and a trigger checks membership before allowing the insert — the same pattern `patient_clinic_access` already uses for clinic access, applied to "was this complaint actually part of this sitting." *Pro:* a genuine closure, not a narrowing — an explicit, checkable record of truth exists. Could also anchor the invoice/session Option B decision (an `invoice_visits` junction, or a direct session-to-invoice relationship, becomes far more natural once "session" is a real row rather than an implicit grouping by date). *Con:* real new schema, real new design questions (what happens to a session started but never submitted — orphan row, needs a status?), and — differently from the per-complaint pre-fill lookup, which I confirmed earlier is genuinely independent of A/B/C — this one is not independent. Building it means engaging with that decision, not sidestepping it.

**Approach 3 — a cheap, narrower check: validate every submitted complaint's `status = 'Active'` at submission time.** No new schema. *Pro:* essentially free, and closes a real, related problem — can't bill against something already `'Completed'`. *Con:* doesn't touch your actual concern — a complaint can be genuinely `'Active'` and still not be the one chosen for today. A different, narrower gap, worth closing regardless, not a substitute.

**Approach 4 — accept the residual risk, detect rather than prevent.** Every relevant table already has `created_by`/`created_at`. Once real auth exists, any write is attributable to a specific person at a specific time — unusual patterns catchable by review after the fact rather than blocked at write time. *Pro:* zero build cost now, proportionate for an internal tool where you likely know everyone with write access. *Con:* prevents nothing — a fundamentally different kind of control, and only works if someone actually builds and uses the review mechanism, which isn't free either, just cheaper than Approach 2.

**Where I'd land, given where the project actually is:** Approach 1 as the near-term baseline — already the direction things were heading, costs nothing extra, proportionate to a not-yet-launched internal tool. Approach 3 alongside it, since it's nearly free and closes a real adjacent gap. Approach 2 I wouldn't build in isolation just to close this one thing — I'd revisit it together with the A/B/C decision, since a real session concept would resolve both "how does billing group visits" and "what was legitimately selected" at once, more coherent than solving this narrower slice first and retrofitting later. Approach 4 as a background layer regardless of what else gets built.Log's current. That closes out what M2-07 opened — whenever you're ready, we can either keep going deeper on any of this or start closing out Module 2 with the actual hands-on exercises from the handover.